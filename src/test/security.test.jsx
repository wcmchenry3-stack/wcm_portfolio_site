...
function getSourceFiles(dir) {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    } else if ((fullPath.endsWith('.jsx') || fullPath.endsWith('.js')) && !fullPath.includes('.test.')) {
      files.push(fullPath)
    }
  }
  return files
...
describe('Security: external links (OWASP A04)', () => {
  it('Home — all target="_blank" anchors have rel="noopener noreferrer"', () => {
    ...
    expect(links.length, 'expected at least one external link on Home page').toBeGreaterThan(0)
    for (const link of links) {
      const rel = (link.getAttribute('rel') ?? '').split(/\s+/)
      expect(rel, `"${link.textContent.trim()}" is missing noopener`).toContain('noopener')
      expect(rel, `"${link.textContent.trim()}" is missing noreferrer`).toContain('noreferrer')
    }
  })
  ...
  it('Resume — all target="_blank" anchors have rel="noopener noreferrer"', () => {
    ...
    const links = container.querySelectorAll('a[target="_blank"]')
    expect(links.length, 'expected at least one external link on Resume page').toBeGreaterThan(0)
    for (const link of links) {
      const rel = (link.getAttribute('rel') ?? '').split(/\s+/)
      expect(rel, `"${link.textContent.trim()}" is missing noopener`).toContain('noopener')
      expect(rel, `"${link.textContent.trim()}" is missing noreferrer`).toContain('noreferrer')
...
describe('Security: images (OWASP A05)', () => {
  it('all images have an alt attribute', () => {
    ...
    expect(images.length, 'expected at least one image on Home page').toBeGreaterThan(0)
...
describe('Security: source code audit (OWASP A03)', () => {
  ...
  it('no dangerouslySetInnerHTML in source files', () => {
    ...
    const violations = files.filter((f) => readFileSync(f, 'utf-8').includes('dangerouslySetInnerHTML'))
    expect(violations, `dangerouslySetInnerHTML found in:\n${violations.join('\n')}`).toHaveLength(0)
  })

  it('no eval() calls in source files', () => {
    const files = getSourceFiles(srcDir)
    const violations = files.filter((f) => /\beval\s*\(/.test(readFileSync(f, 'utf-8')))
    expect(violations, `eval() found in:\n${violations.join('\n')}`).toHaveLength(0)
  })
})
